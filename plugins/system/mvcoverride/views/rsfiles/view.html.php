...
include_once JPATH_SITE . '/components/com_rsfiles/helpers/rsfiles.php';
...
class rsfilesViewRsfiles extends JViewLegacy
{
	public function display($tpl = null)
	{
		...
		$this->tagMenu     = rsfilesHelper::isTagMenu();

		if ($this->config->modal == 2)
		{
			JHtml::script('com_rsfiles/jquery.magnific-popup.min.js', array('relative' => true, 'version' => 'auto'));
			JHtml::stylesheet('com_rsfiles/magnific-popup.css', array('relative' => true, 'version' => 'auto'));
		}
		...
		if ($layout == 'briefcase')
		{
			...
			{
				$this->app->enqueueMessage(JText::_('COM_RSFILES_PLEASE_LOGIN'), 'error');
				$this->app->redirect(JRoute::_('index.php?option=com_users&view=login&return=' . base64_encode($uri), false));
			}
		...
		else if ($layout == 'search')
		{
			...
			$this->filter    = trim($this->app->input->getString('filter_search', ''));
		...
		else if ($layout == 'upload')
		{
			...
			$this->curentfilesno = rsfilesHelper::getCurrentFilesNo($this->briefcase);
			$this->currentquota  = $this->briefcase ? rsfilesHelper::getFoldersize($root . $this->ds . $user_folder) : rsfilesHelper::getUserFilesSize();
			...
			$uri   = JUri::getInstance();
			$clone = clone $uri;
			$clone->setVar('single', 1);
			$this->singleupload = $clone->toString();
		...
		else if ($layout == 'download')
		{
			...
			{
				if (JFactory::getApplication()->input->get('relatedfile'))
				{
					unset($this->currentRel);
				}
				$checkPath   = empty($this->currentRel) ? $this->path : $this->currentRel . rsfilesHelper::ds() . $this->path;
			...
			if (JFactory::getApplication()->input->get('relatedfile'))
			{
				$this->current = $root_path = $this->config->download_folder;
			}
			$current_path = $this->current . $this->ds . $this->path;
			...
			$this->file->fullpath = $element->fullpath;
			...
			$previous       = $this->file->FileType && $previous == 'root' ? '' : $previous;
		...
		else if ($layout == 'tags')
		{
			...
			$this->items      = $this->get('TaggedItems');
		...
		// If displaying in module honor the number of files to display
		if (JFactory::getApplication()->scope == 'mod_rsfiles_newest')
		{
			$number_of_results = JFactory::getApplication()->getUserState('number_of_results', 5);
			$this->items       = array_slice($this->items, 0, $number_of_results);
		}

		if (JFactory::getApplication()->scope == 'mod_rsfiles_list_tags')
		{
			$mod_rsfiles_list_tags_params = JFactory::getApplication()->getUserState('mod_rsfiles_list_tags.params');
			$number_of_results            = $mod_rsfiles_list_tags_params->get('display', '7');

			// remove any folders from the list
			function test_for_file($item)
			{
				return $item->type == 'file';
			}

			$files       = array_filter($this->items, "test_for_file");
			$this->items = array_slice($files, 0, $number_of_results);
		}
